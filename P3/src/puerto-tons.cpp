...
void fin_carga()
{
    ...
    acum_at_cargando += (reloj - tdus_at) * (num_atraques - atraques_libres - encola_sal);
    tdus_at = reloj;
    switch (nodo.reg_cola.tipo)
    {
        case 0:
            toneladas_cargadas += 1000.0;
            break;
        case 1:
            toneladas_cargadas += 2000.0;
            break;
        case 2:
            toneladas_cargadas += 3000.0;
            break;
    }
...
void fin_simulacion()
{
    ...
    informe[cont_simu][11] = toneladas_cargadas;
}
...
void generador_informes(int simulaciones)
{
    ...
    printf("\nTotal de toneladas cargadas: media(%f), dt(%f)\n\n", media[11], dt[11]);
}
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < simulaciones; i++)
        informe[i] = (float *)malloc(12 * sizeof(float));
